...
$SETSTATUS 2 "$(/usr/local/bin/sb-vol)" &

secs=0
while true; do

    $SETSTATUS 0 "$(/usr/local/bin/sb-clock)" &
    $SETSTATUS 3 "$(/usr/local/bin/sb-temp)" &
    $SETSTATUS 5 "$(/usr/local/bin/sb-memory)" &
    $SETSTATUS 6 "$(/usr/local/bin/sb-cpu)" &
    ...
    if [ $((secs % 3600)) = 0 ]; then
    	$SETSTATUS 4 "$(/usr/local/bin/sb-disk)" &
	$SETSTATUS 1 "$(/usr/local/bin/sb-updates)" &
